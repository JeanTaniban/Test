...
print_new_lines() {
  local file="$1" prefix="$2" previous="$3" current
...
log "readiness=cloudflare-doh-plus-curl-resolve local_dns=unused"
...
for _ in $(seq 1 30); do
  ...
  if ! kill -0 "$server_pid" 2>/dev/null; then
    ...
    tail -n 100 "$SERVER_LOG" | sed 's/^/[server] /' | tee -a "$SUPERVISOR_LOG"
    exit "$rc"
  fi
  sleep 0.5
done
...
for _ in $(seq 1 90); do
  ...
  if [[ -n "$url" ]] && grep -q 'Registered tunnel connection' "$TUNNEL_LOG"; then
    registered=1
    log "cloudflare connector registered url=$url"
    break
  fi
...
if (( registered == 0 )) || [[ -z "$url" ]]; then
  log "ERROR Quick Tunnel never reached registered state"
  tail -n 160 "$TUNNEL_LOG" | sed 's/^/[cloudflared] /' | tee -a "$SUPERVISOR_LOG"
  cleanup EXIT
  exit 1
fi
...
echo
printf '=== PUBLIC READINESS (sans DNS local Termux) ===\n'
printf 'URL attribuee: %s\n' "$url"
printf 'Methode: Cloudflare DoH via IP litterale + HTTPS forcee avec curl --resolve.\n\n'

public_ready=0
if wait_for_quick_tunnel_public_ready "$url" "$PUBLIC_READY_WAIT_SECONDS" ready > >(tee -a "$SUPERVISOR_LOG") 2>&1; then
  public_ready=1
  log "public route READY url=$url"
else
  log "public route NOT READY after ${PUBLIC_READY_WAIT_SECONDS}s url=$url"
fi

if (( public_ready == 1 )); then
  printf '\n=== MODE DEBUG: 60 secondes de surveillance active ===\n'
  printf 'URL publique verifiee: %s\n' "$url"
  if [[ "$AUTO_OPEN_BROWSER" != "0" ]]; then
    open_url "$url"
  fi
else
  printf '\n=== MODE DEBUG: surveillance sans ouverture Chrome ===\n'
  printf 'Le tunnel reste lance; le resolver DNS local Termux n a pas ete utilise.\n'
fi
...
for second in $(seq 1 60); do
  ...
  if ! kill -0 "$server_pid" 2>/dev/null; then
    ...
    tail -n 180 "$SERVER_LOG" | sed 's/^/[server-final] /' | tee -a "$SUPERVISOR_LOG"
  ...
  if ! kill -0 "$tunnel_pid" 2>/dev/null; then
    ...
    tail -n 220 "$TUNNEL_LOG" | sed 's/^/[cloudflared-final] /' | tee -a "$SUPERVISOR_LOG"
  ...
  if (( second % 5 == 0 )); then
    ...
    public_ip="$(quick_tunnel_public_health "$url" 2>/dev/null || true)"
    public_state="not-ready"
    [[ -n "$public_ip" ]] && public_state="200-via-$public_ip"
    log "heartbeat t=${second}s $(proc_summary node "$server_pid") $(proc_summary cloudflared "$tunnel_pid") local_health=$local_code public_route=$public_state local_dns=unused"
  fi
  sleep 1
done
